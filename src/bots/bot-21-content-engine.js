...
const { Client: ResendClient } = require('resend');
...
async function generateScript(openai, pillar) {
  ...
  let words = script.split(/\\s+/).length;
  if (words > 28) {
    const trimResponse = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [{
        role: 'user',
        content: 'Rewrite this in EXACTLY 28 words: ' + script
      }],
      max_tokens: 50
    });
...
async function sendApprovalEmail(videoId, script, pillar) {
  ...
  const html = '<h2>New Content Script Generated</h2><p><strong>Pillar:</strong> ' + pillar + '</p><p><strong>Script:</strong></p><blockquote style="background: #f5f5f5; padding: 12px; border-left: 3px solid #0066cc;">' + script + '</blockquote><p><a href="' + watchUrl + '" style="display: inline-block; padding: 10px 20px; background: #0066cc; color: white; text-decoration: none; margin-right: 10px;">Watch</a><a href="' + approveUrl + '" style="display: inline-block; padding: 10px 20px; background: #28a745; color: white; text-decoration: none; margin-right: 10px;">Approve</a><a href="' + rejectUrl + '" style="display: inline-block; padding: 10px 20px; background: #dc3545; color: white; text-decoration: none;">Reject</a></p>';
...
async function run() {
  ...
  const schedules = [
    { hour: 7, minute: 0, platform: 'youtube' },
    { hour: 9, minute: 0, platform: 'instagram' },
    { hour: 11, minute: 0, platform: 'x' },
    { hour: 13, minute: 0, platform: 'instagram' },
    { hour: 15, minute: 0, platform: 'youtube' },
    { hour: 17, minute: 0, platform: 'x' },
    { hour: 19, minute: 0, platform: 'instagram' },
    { hour: 21, minute: 0, platform: 'x' }
  ];
  ...
  for (const sched of schedules) {
    ...
    try {
      ...
      const sent = await sendApprovalEmail(video.id, script, pillar);
      if (sent) emailsSent++;
      else emailsFailed++;
...
run().catch(err => {
  console.error('[BOT-21] [ERROR]', err.message);
  process.exit(1);
});
